...
"""----------------------------------------------------------"""
# cruising speed control constants
...
first_controller_output = 0.01  # first value of integral
...
Td = Tu / 8
"""----------------------------------"""
# Upper and Lower limits on operation output
control_out_max = 0.5
control_out_min = -0.5
"""----------------------------------------------------------"""
...
def speed_control(val, auto_drive):
    ...
    velocity = auto_drive.getCurrentSpeed()  # velocity (m/s)
...
def follow_lane_PID(position, targetPosition, auto_drive):
    ...
    # estimated_position = Storage.loadData("estimated_position", file_path)
    # if estimated_position is not None:
    #     if estimated_position > position:
    #         position = position + (estimated_position - position)
    ...
    if steering_angle > control_out_max:  # check upper limit
        steering_angle = control_out_max
        integral_of_error = integral_of_error - e * delta_t  # anti-reset windup
        Storage.storeData("prev_integral_of_error", round(integral_of_error, 2), file_path)
    if steering_angle < control_out_min:  # check lower limit
        steering_angle = control_out_min
        integral_of_error = integral_of_error - e * delta_t  # anti-reset windup
        Storage.storeData("prev_integral_of_error", round(integral_of_error, 2), file_path)
    # # to prevent derivative kick the value of the error changes suddenly whenever the set point is adjusted.
