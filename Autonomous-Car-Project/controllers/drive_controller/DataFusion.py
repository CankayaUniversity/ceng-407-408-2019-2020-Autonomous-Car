...
def obj_center(dist_sensors, dist_sensor_data, auto_drive):
    """ Finds necessarily value to modify lane center according to distance sensor data
     :param auto_drive:
    ...
    try:
        if dist_sensor_data is not None:
            for key, value in dist_sensor_data.items():
                ...
                # value is decreasing by getting closer to object
                if value < (dist_sensors[key].getMaxValue() * 0.5):  # closer than half of the ray
...
def check_sides(dist_sensor_data: dict) -> Optional[str]:
    """ Check right and left sides of the car for available side to lane changing.
...
def change_lane(display_front: object, front_cams: dict, auto_drive: vars, gps: float, status: str, side: str,
                respond_dict: dict) -> float:
    """ Call lane_management.py to find center of the lane with image processing.
    ...
    try:
        if side is None:
            if not isnan(auto_drive.getCurrentSpeed()):
                # if status == "front":
                #     if not isnan(auto_drive.getCurrentSpeed()):
                #         val = auto_drive.getCurrentSpeed() - 2
                if status == "emergency":
...
def main(auto_drive: vars, gps: float, dist_sensor_data: dict, lidar_data: object, emergency: bool,
         display_front: object, front_cams: dict, dist_sensors: object, VA_order: int,
         respond_dict: dict, gps_total: object, axis: int) -> None:
    ...
    """
     Main function to call necessarily parts
        :type front_cams: dict
        :param lidar_data:
        :param front_cams: front camera instances
        :param dist_sensors : Distance sensors
        :type dist_sensors: DistanceSensor() object under Drive()
        :param respond_dict: dictionary to send value VA process
        :param gps: calculated value of gps device
        ...
        :param display_front: Display screen of front
        :type display_front: Display under Controller()
        :param emergency: Emergency status comes from receiver of the car in drive_controller.py
        :type emergency: int
        :param dist_sensor_data: distance sensor variable comes from object recognition
        :type dist_sensor_data dict
        :param VA_order: order comes from voice assistant
    ...
    # lane_width = Storage.loadData("lane_width", lane_file_path)
    Log.clear()
    error_Log.clear()
    global RANGE_FRONT, RANGE_REAR
    try:
